...
class Vgg19Face(torch.nn.Module):
    def __init__(self, requires_grad=False):
        ...
        checkpoint = torch.load('models/checkpoints/VGG19_FACE_checkpoint.pth.tar')
...
class Vgg19Gender(torch.nn.Module):
    def __init__(self, requires_grad=False):
        super(Vgg19Gender, self).__init__()
        vgg_pretrained_features = models.vgg19()
        #print(vgg_pretrained_features)
        vgg_pretrained_features.eval()
        checkpoint = torch.load('models/checkpoints/VGG19_Gender_checkpoint.pth.tar')
        state_dict =checkpoint['state_dict']
        #print(state_dict)
        new_state_dict = OrderedDict()
        index_layer=0
...
class VGGGenderLoss(nn.Module):
    def __init__(self, gpu_ids):
        super(VGGGenderLoss, self).__init__()        
        self.vgg = Vgg19Gender().cuda()
        self.criterion = nn.L1Loss()
        self.weights = [1.0/32, 1.0/16, 1.0/8, 1.0/4, 1.0]        
